...
def register_prov_routes(app, registry: PluginRegistry, get_user):
    ...
    @app.get(
        ...
        description="Interactive visualization of the full provenance graph. Use ?include_system_activities=false to hide system activities.",
        response_class=HTMLResponse,
    )
    async def get_prov_graph(
        dossier_id: UUID,
        include_system_activities: bool = True,
        user: User = Depends(get_user),
    ):
        session_factory = get_session_factory()
        async with session_factory() as session:
            ...
            # Build set of activity IDs to skip
            ...
            # Also skip entities generated by skipped activities
            if skipped_activity_ids:
                all_entities = [e for e in all_entities if e.generated_by not in skipped_activity_ids]
            ...
            for act in activities:
                ...
                # wasInformedBy edges
                if act.informed_by and act.informed_by not in skipped_activity_ids:
                    edges.append({
