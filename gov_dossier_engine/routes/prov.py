...
def register_prov_routes(app, registry: PluginRegistry, get_user):
    ...
    @app.get(
        ...
        description="Interactive visualization of the provenance graph. Use ?include_system_activities=true to include system activities.",
    ...
    async def get_prov_graph(
        ...
        async with session_factory() as session:
            ...
            if activity_view_mode != "all":
                for act in activities:
                    ...
                    if activity_view_mode == "own":
                        ...
                        # Activities touching visible entities OR own
                        for used in used_by_activity.get(act.id, []):
            ...
            # Access-filtered entities are already handled by visible_types above
            if not include_system_activities:
                system_skipped = set(
